 ...
 *
 * NOTE: this is never called directly. Only by parent 'flow' reducer (defined
 * below) when dealing with a LIST action
 */
function flowList(state = {
  /**
   ...
   * then recursively return an _additional_ flowList reducer.
   *
   * Otherwise, return the actual flow lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional flowList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: flowList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_FLOW_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_FLOW_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_FLOW_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
      ...
      case Actions.REMOVE_FLOW_FROM_LIST:
        idArray = [...state.items]
      ...
      case Actions.SET_FLOW_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_FLOW_PAGINATION: {
        return {
          ...state
...
/**
 * Primary flow reducer -
 *
 * This is the single source of truth for all things 'flow' related within the
...
function flow(state = {
  ...
  /**
   * "byId" is an object map of all flow items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a flow, the single flow
  ...
  /**
   * "lists" corresponds to individual instances of the flowList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_FLOW: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_FLOW: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUESTFLOWSCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVEFLOWSCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     * SINGLE FLOW ACTIONS
     */
    case Actions.REQUEST_SINGLE_FLOW: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_FLOW: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_FLOW_TO_MAP: {
      // add this flow to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.SET_SELECTED_FLOW: {
    ...
    case Actions.REQUEST_CREATE_FLOW: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_FLOW: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_FLOW: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_FLOW: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_FLOW: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_DELETE_FLOW: {
      if(action.success) {
    ...
    case Actions.INVALIDATE_SELECTED_FLOW: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
    ...
    case Actions.SET_FLOW_PAGINATION: {
      nextState = {
        ...
        , lists: {
          ...state.lists
          , [action.listArgs[0]]: flowList(state.lists[action.listArgs[0]] || {}, action)
        }
      }
      break;
    }
